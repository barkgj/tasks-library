...
function nxs_task_instance_do_create_task_instance($then_that_item, $taskid, $taskinstanceid)
{
	...
	if ($state == "STARTED")
	{
		...
		foreach ($then_that_item as $key => $val)
		{
			if (is_string($key) && is_string($val))
			{
				...
				{
					$then_that_item[$key] = nxs_filter_translatesingle($val, "{{", "}}", $inputparameters);
				}
			}
		}
		...
		$create_taskid = nxs_filter_translatesingle($create_taskid, "{{", "}}", $inputparameters);
		...
		$create_taskid = nxs_filter_translatesingle($create_taskid, "{{", "}}", $extraparameters);
		
		if (nxs_stringcontains($create_taskid, "{"))
		{
		...
		$tasktitle = nxs_tasks_gettaskstitle($create_taskid);
		$processingtype = nxs_tasks_getprocessingtype($create_taskid);
		...
		// dont use a GET request as its possible the URL will be too longer depending on the inputparameters
		
		$args = array();
		$args["url"] = "https://global.nexusthemes.com/api/1/prod/create-task-instance/";
		$args["timeoutsecs"] = 999;
		$args["method"] = "POST";
		$args["postargs"] = array();
		$args["postargs"]["nxs"] = "businessprocess-api";
		$args["postargs"]["nxs_json_output_format"] = "prettyprint";
		
		// override the businessprocesstaskid
		$args["postargs"]["businessprocesstaskid"] = $create_taskid;
		
		foreach ($parameters as $k2 => $val)
		{
			$args["postargs"][$k2] = $val;
		}
	
		$args["postargs"]["createdby_taskid"] = $taskid;
		$args["postargs"]["createdby_taskinstanceid"] = $taskinstanceid;
		...
		if ($doit)
		{
			...
			if ($action_result["result"] != "OK") 
			{
			 $result = array
				(
					"result" => "NACK",
					"error" => "unable to fetch action_url; $action_url",
				);
				return $result;
			}
		...
		else
		{
	  	...
	  	$action_url = $currenturl;
			...
			if (true) // $should_render_required_fields)
			{
				...
				$meta = nxs_tasks_getreflectionmeta($create_taskid);
			  ...
			  $html .= "'businessprocesstaskid': '{$create_taskid}',";
				...
				$sticky = nxs_task_getstickyparameters();
				foreach ($parameters as $k2 => $val)
				{
			  ...
			  foreach ($required_fields as $required_field)
				...
				if (nxs_stringcontains($happyflow_behaviour, "end_task_instance"))
				{
					$html .= "request.endparentinstance = 'true';";
					//
				}
				
				if (nxs_stringcontains($happyflow_behaviour, "start_child_task_instance"))
				{
			  ...
			  $html .= "console.log('response:');";
				...
				if (nxs_stringcontains($happyflow_behaviour, "redirect_to_child_instance"))
				{
				...
				if (nxs_stringcontains($happyflow_behaviour, "redirect_to_instances_overview"))
				{
				...
				if (nxs_stringcontains($happyflow_behaviour, "reload_gui_current_page"))
				{
					$marker = $then_that_item["marker"];
					$random_reload_gui_token = time();
				  $currenturl = "https://global.nexusthemes.com/?nxs=task-gui&page=taskinstancedetail&taskid={$taskid}&taskinstanceid={$taskinstanceid}&random_reload_gui_token={$random_reload_gui_token}#{$marker}";
				...
				if (nxs_stringcontains($happyflow_behaviour, "show_link_to_childinstance"))
				{
					$html .= "var child_url = 'https://global.nexusthemes.com/?nxs=task-gui&page=taskinstancedetail&taskid=' + child_taskid + '&taskinstanceid=' + child_taskinstanceid;";
