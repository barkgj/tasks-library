...
function nxs_task_instance_do_send_endclientslive_billing_reminder($then_that_item, $taskid, $taskinstanceid)
{
	...
	if ($state == "STARTED")
	{
		...
		$mail_url = nxs_addqueryparametertourl_v2($mail_url, "mailtemplate", "8", true, true);
		...
		if ($doit)
		{
			...
			{
				$result["console"][] = "Error enqueue-ing sending mail";
				$result["result"] = "NACK";
			...
			$result["console"][] = "Enqueueing mail succesfully";
		}
		else
		{
	  	...
	  	$action_url = $currenturl;
			...
			$result["console"][] = "<a href='{$action_url}#$marker'>Click here to enqueue email</a>";
		}
	}
	else
  {
