...
function nxs_task_instance_do_pick_expired_license($then_that_item, $taskid, $taskinstanceid)
{
	$result["console"][] = "PICK EXPIRED LICENSE";
	$result["console"][] = "-----------";
	...
	if ($licenseid == "")
	{
		...
		foreach ($pieces as $piece)
		{
			...
			$result["console"][]= "licenseid: {$piece} (sitedomain: $sitedomain)";
		...
		if (count($pieces) == 0)
		{
			$result["console"][] = "NO POSSIBLE EXPIRED LICENSES FOUND (expiredlicenses is empty)";
		}
	}
	else
	{
