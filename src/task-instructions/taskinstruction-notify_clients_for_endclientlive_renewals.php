...
function nxs_get_remindergroups_for_endclientslive()
{
...
function nxs_task_instance_do_notify_clients_for_endclientlive_renewals($then_that_item, $taskid, $taskinstanceid)
{
	...
	if ($state == "STARTED")
	{
		$remindergroups = nxs_get_remindergroups_for_endclientslive();
		...
		foreach ($remindergroups as $remindergroup)
		{
			...
			$listlicenses_url = nxs_addqueryparametertourl_v2($listlicenses_url, "requirement_in_studio", "endclientslive", true, true);		
			...
			foreach ($listlicenses_result["licenses"] as $license)
			{
				...
				{
					//
					$result["console"][] = "skipping creating of instance (dry run, use createinstances=true to create the instances)";
				}
			}
