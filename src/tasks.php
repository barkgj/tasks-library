...
final class tasks
{
	...
	{
		$result = functions::getsitedatafolder() . "/tasks-instances/";
		echo "tasks::getbasefolder:{$result}";
		die();
		return $result;
	}

	public static function gettaskpath($taskid)
	{
		if ($taskid == "") { functions::throw_nack("taskid not set?"); }
		$taskbasefolder = tasks::getbasefolder();
		$result = "{$taskbasefolder }/{$taskid}.json";
		return $result;
	}
	...
	public static function createtaskinstance($taskid, $assigned_to, $createdby_taskid, $createdby_taskinstanceid, $mail_assignee, $inputparameters)
	{
		

		if ($taskid == "") { functions::throw_nack("taskid not set"); }
		
		// filter out unwanted inputparameters for the child, even if those are specified
		...
		{
			unset($inputparameters[$unwantedfield]);
		}

		// instead of modelmanager, fetch using datasink
		$args = array
		(
		...
		$task = entity::getentitymetadataraw($args);
		var_dump($task);
		...
		$properties = $g_modelmanager->getmodeltaxonomyproperties($a);
		...
		{
			$parentinputparameters = tasks::getinstanceinputparameters($createdby_taskid, $createdby_taskinstanceid);
			...
			$sticky_inputparameters = tasks::getstickyparameters();
		...
		if (!$r)
		{
			functions::throw_nack("unable to store task instance");
		}
	...
	public static function getrendered_taskinstances($taskinstances, $renderargs)
	{
		$entries = array();
		foreach ($taskinstances as $taskinstance)
		{
			...
			$instancemeta = tasks::getinstance($taskid, $taskinstanceid);
		...
		foreach ($entries as $entry)
		{
			...
			$o_inputparameters = $renderargs["o_inputparameters"];
	...
	public static function wakeparentoftaskinstance($taskid, $taskinstanceid)
	{
		...
		$parentmeta = tasks::getinstance($parent_taskid, $parent_taskinstanceid);
		...
		tasks::updateinstance($parent_taskid, $parent_taskinstanceid, $parentmeta);
	}
	...
	public static function gettaskinstancelookup($taskid, $taskinstanceid, $orig_args = array())
	{
		...
		$persistent_instance_fields = $instancemeta["inputparameters"];
	...
	public static function updateinstance($taskid, $taskinstanceid, $taskinstancemeta)
	{
		if ($taskid == "") { functions::throw_nack("taskid not set"); }
		if ($taskinstanceid == "") { functions::throw_nack("taskinstanceid not set"); }
		...
		$instances[$taskinstanceid] = $taskinstancemeta;
		$string = json_encode($instances);
		$path = tasks::gettaskpath($taskid);
		$r = file_put_contents($path, $string, LOCK_EX);
		if ($r === false)
		{
			functions::throw_nack("updateinstance; not able to write?");
		}
	...
	public static function appendinputparameter_for_taskinstance($taskid, $taskinstanceid, $key, $val)
	{
		...
		$inputparameterstoappend[$key] = $val;
		tasks::appendinputparameters_for_taskinstance($taskid, $taskinstanceid, $inputparameterstoappend);
	}
	...
	public static function getstoredtaskinstructionresult($taskid, $taskinstanceid, $taskinstructiontype)
	{
		...
		$inputparameters = tasks::getinstanceinputparameters($taskid, $taskinstanceid);
		$json = $inputparameters[$key];
		if ($json == "")
		{
			$result = "";
		}
	...
	public static function appendinputparameters_for_taskinstance($taskid, $taskinstanceid, $inputparameterstoappend)
	{
		$basefolder = tasks::getbasefolder();
		$path = "{$basefolder}/{$taskid}.json";
		$string = file_get_contents($path);
		$meta = json_decode($string, true);
		foreach ($inputparameterstoappend as $name => $value)
		{
			$meta[$taskinstanceid]["inputparameters"][$name] = $value;
		}
		$string = json_encode($meta);
		$r = file_put_contents($path, $string, LOCK_EX);
		if ($r === false) { functions::throw_nack("error updating task instance"); }
	...
	public static function deleteinputparameters_for_taskinstance($taskid, $taskinstanceid, $inputparameters)
	{
		$basefolder = tasks::getbasefolder();
		$path = "{$basefolder}/{$taskid}.json";
		$string = file_get_contents($path);
		...
		{
			unset($meta[$taskinstanceid]["inputparameters"][$inputparameter]);
		}
		$string = json_encode($meta);
		$r = file_put_contents($path, $string, LOCK_EX);
	...
	public static function deleteinput_for_taskinstance($taskid, $taskinstanceid, $inputparameter)
	{
		...
		$result = tasks::deleteinputparameters_for_taskinstance($taskid, $taskinstanceid, $inputparameters);
		return $result;
	}
	...
	{
		$path = tasks::getpath($taskid);
		$result["size"] = filesize($path);
		$instances = tasks::getinstances($taskid);
		$result["count"] = count($instances);
		return $result;
	}

	public static function getpath($taskid)
	{
		$result = functions::getsitedatafolder() . "/tasks-instances/{$taskid}.json";
		return $result;
	}

	public static function getinstances($taskid)
	{
		$path = tasks::getpath($taskid);
		$string = file_get_contents($path);
	...
	public static function getinstance($taskid, $taskinstanceid)
	{
		...
		{
			$instances = tasks::getinstances($taskid);
			if (isset($instances[$taskinstanceid]))
			{
				$result = $instances[$taskinstanceid];
	...
	public static function getinstancestate($taskid, $taskinstanceid)
	{
		$instance = tasks::getinstance($taskid, $taskinstanceid);
	...
	{
		$meta = tasks::getinstance($taskid, $taskinstanceid);
		$meta["finished_instruction_pointer"] = $finished_instruction_pointer;
		tasks::updateinstance($taskid, $taskinstanceid, $meta);
	}
	...
	public static function batch_reverttorequiredinputparameters($taskid, $taskinstanceids)
	{
		if ($taskid == "") { functions::throw_nack("tasks_batch_reverttorequiredinputparameters; taskid not set"); }
		...
		$i = 1;
		error_log("tasks_batch_reverttorequiredinputparameters; $taskid; start(batchsize; $batchsize)");
		
		foreach ($taskinstanceids as $taskinstanceid)
		{
			error_log("tasks_batch_reverttorequiredinputparameters; executing $taskid; $taskinstanceid; ({$i}/{$batchsize}");
			tasks::reverttorequiredinputparameters($taskid, $taskinstanceid);
		...
		error_log("tasks_batch_reverttorequiredinputparameters; $taskid; end");
		
		$result = array();
		return $result;
	}

	public static function reverttorequiredinputparameters($taskid, $taskinstanceid)
	{
		$existinginputparameters = tasks::getinstanceinputparameters($taskid, $taskinstanceid);
		
		$meta = tasks::getreflectionmeta($taskid, $taskinstanceid);
		$required_fields = $meta["required_fields"];
		
		error_log("tasks::reverttorequiredinputparameters; $taskid, $taskinstanceid; required fields:" . json_encode($required_fields));
		
		$fields_to_be_removed = array();
		foreach ($existinginputparameters as $existinginputparameter => $existinginputparameterval)
		{
		...
		error_log("tasks::reverttorequiredinputparameters; $taskid, $taskinstanceid; fields to be removed:" . json_encode($fields_to_be_removed));
		
		tasks::deleteinputparameters_for_taskinstance($taskid, $taskinstanceid, $fields_to_be_removed);
	...
	public static function getinstanceinputparameters($taskid, $taskinstanceid)
	{
		$instance = tasks::getinstance($taskid, $taskinstanceid);
		$result = $instance["inputparameters"];
		$result["taskid"] = $taskid;
		$result["taskinstanceid"] = $taskinstanceid;
		return $result;
	}
	...
	public static function getstacktracepreviousgeneration($taskid, $taskinstanceid)
	{
		...
		foreach ($stacktrace_current_generation as $frame)
		{
			...
			{
				$repliesto_taskid = $frame["inputparameters"]["repliesto_taskid"];
				$repliesto_taskinstanceid = $frame["inputparameters"]["repliesto_taskinstanceid"];
	...
	public static function getstacktrace($taskid, $taskinstanceid, $args)
	{
		...
		if ($includecurrent)
		{
			$result[] = tasks::getinstance($taskid, $taskinstanceid);
		}
	...
	public static function getstacktrace_internal($taskid, $taskinstanceid, $maxdepth = 100, &$resultsofar = array())
	{
		...
		$taskmeta = tasks::getinstance($taskid, $taskinstanceid);	
		...
		// createdby_taskid and createdby_taskinstanceid were stored in inputparameters
		if ($parent_taskid == "" || $parent_taskinstanceid == "")
		{
			$inputparameters_current = tasks::getinstanceinputparameters($taskid, $taskinstanceid);
			$parent_taskid = $inputparameters_current["createdby_taskid"];
			$parent_taskinstanceid = $inputparameters_current["createdby_taskinstanceid"];
		}
		
		if ($parent_taskid != "" && $parent_taskinstanceid != "")
		{
			tasks::getstacktrace_internal($parent_taskid, $parent_taskinstanceid, $maxdepth - 1, $resultsofar);
			$resultsofar[] = tasks::getinstance($parent_taskid, $parent_taskinstanceid);
		}
	}
	...
	{
		$result = array();
		$this_meta = tasks::getinstance($taskid, $taskinstanceid);
	...
	public static function gettaskinstancemetarecursiveoffspring_internal($taskid, $taskinstanceid, $maxdepth = 2)
	{
		...
		foreach ($children_of_this as $child_of_this)
		{
			...
			$child_meta = tasks::getinstance($child_taskid, $child_taskinstanceid);
			...
			if (file_exists($possiblefilepath))
			{
				...
				foreach ($instances_data as $id => $instance_meta)
				{
					...
					if ($ismatch)
					{
						...
						{
							if ($instance_meta["inputparameters"][$k] != $v)
							{
								$ismatch = false;
								break;
							}
	...
	public static function search_evaluate_if_this($if_this, $taskid, $taskmeta)
	{
		...
		else if ($if_this_type == "true_if_inputparameter_has_required_value_for_key")
		{
			...
			$value = $taskmeta["inputparameters"][$key];
	...
	public static function createtaskinstance_byinvokingapi($taskidofinstancetocreate, $inputparameters, $createdby_taskid, $createdby_taskinstanceid)
	{
	...
	// free key
	public static function getunallocatedinputparameter($base_of_key, $inputparameters, $maxindex = 99)
	{
		$result = false;
		$index = 0;
		while ($index < $maxindex)
		{
			...
			if (!isset($inputparameters[$possible_key]))
			{
				$result = $possible_key;
				break;
			}
	...
	public static function endtaskinstance($taskid, $taskinstanceid)
	{
		...
		tasks::updateinstance($taskid, $taskinstanceid, $instancemeta);
	...
	public static function batch_resetnonerequiredinputparameters($taskid, $taskinstanceids)
	{
		if ($taskid == "") { functions::throw_nack("tasks_batch_resetnonerequiredinputparameters; taskid not set"); }
		...
		$batchsize = count($taskinstanceids);
		$i = 1;
		error_log("tasks_batch_resetnonerequiredinputparameters; $taskid; start(batchsize; $batchsize)");
		
		foreach ($taskinstanceids as $taskinstanceid)
		{
			error_log("tasks_batch_resetnonerequiredinputparameters; executing $taskid; $taskinstanceid; ({$i}/{$batchsize}");
			tasks::batch_resetnonerequiredinputparameters($taskid, $taskinstanceid);
